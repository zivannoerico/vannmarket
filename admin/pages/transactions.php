<?php
// admin/pages/transactions.php — Kelola Transaksi
...
if ($search)        $where .= " AND (t.game_account_id LIKE '%". $conn->real_escape_string($search) ."%' OR t.trx_id LIKE '%". $conn->real_escape_string($search) ."%')";

$trxs = $conn->query("SELECT t.*, g.game_name, dp.package_name, pm.method_name
...
<div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;align-items:center;">
  <form method="GET" style="display:contents;">
    ...
    <input type="text" name="q" value="<?= esc($search) ?>" placeholder="Cari ID / User ID..." style="padding:9px 14px;background:#222;border:1px solid #333;border-radius:8px;color:#fff;font-family:inherit;font-size:14px;outline:none;width:200px;">
...
<div class="section-card">
  ...
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
          <th>#ID</th><th>Game</th><th>Paket</th><th>User ID Game</th>
          <th>Pembayaran</th><th>Total</th><th>Status</th><th>Tanggal</th><th>Aksi</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          ...
          <td style="font-family:monospace;"><?= esc($t['game_account_id']) ?></td>
