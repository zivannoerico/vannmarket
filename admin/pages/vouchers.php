...
if (isset($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        header("Location: ?page=vouchers&msg=Voucher+dihapus."); exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$code) {
        $msg = 'Kode voucher wajib diisi.'; $msg_type = 'error';
    } else {
        ...
        if ($chk->get_result()->num_rows > 0) {
            $msg = "Kode voucher sudah digunakan."; $msg_type = 'error';
        } else {
            if ($action === 'add') {
                $stmt = $conn->prepare("INSERT INTO vouchers (voucher_code,game_id,discount_pct,valid_from,valid_until,created_at) VALUES (?,?,?,?,?,NOW())");
                $stmt->bind_param("sidss", $code, $game_id, $discount_pct, $valid_from, $valid_until);
                if ($stmt->execute()) {
                    header("Location: ?page=vouchers&msg=Voucher+berhasil+dibuat."); exit;
                } else {
                ...
                $stmt->bind_param("sidsssi", $code, $game_id, $discount_pct, $valid_from, $valid_until, $vid);
                if ($stmt->execute()) {
                    header("Location: ?page=vouchers&msg=Voucher+berhasil+diperbarui."); exit;
                } else {
                    $msg = 'Gagal: '.$conn->error; $msg_type = 'error';
                }
            }
  ...
  <form method="POST">
    ...
    <div class="form-grid">
      <div class="field">
        ...
        <input type="text" name="voucher_code" value="<?= esc($edit['voucher_code'] ?? '') ?>" placeholder="Contoh: PROMO10" required oninput="this.value=this.value.toUpperCase()">
      ...
      <div class="field">
        ...
        <select name="game_id">
          ...
          <?php $games_list->data_seek(0); while ($gm = $games_list->fetch_assoc()): ?>
          <option value="<?= $gm['game_id'] ?>" <?= (isset($edit['game_id']) && $edit['game_id'] == $gm['game_id']) ? 'selected' : '' ?>><?= esc($gm['game_name']) ?></option>
          <?php endwhile; ?>
        </select>
      </div>
      <div class="field">
        <label>Diskon (%)</label>
        <input type="number" name="discount_pct" value="<?= $edit['discount_pct'] ?? 0 ?>" step="0.1" min="0" max="100">
      </div>
      <div class="field">
    ...
    <div style="padding:0 22px 22px;">
      <button type="submit" class="btn btn-primary"><?= $edit ? '💾 Simpan Perubahan' : '➕ Buat Voucher' ?></button>
    </div>
  </form>
</div>
<div class="section-card">
  <div class="section-header">
    <h2>🎫 Daftar Voucher (<?= $vouchers->num_rows ?>)</h2>
    <span style="font-size:12px;color:#888;">Voucher aktif otomatis tampil di halaman user</span>
  </div>
  <div class="table-wrap">
    <table>
      ...
      <tbody>
        ...
        <?php else: while ($v = $vouchers->fetch_assoc()):
        ...
        <tr>
          ...
          <td>
            <div class="cell-actions">
              ...
              <a href="?page=vouchers&delete=<?= $v['voucher_id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Hapus voucher ini?')">Hapus</a>
            </div>
          </td>
        </tr>
        <?php endwhile; endif; ?>
